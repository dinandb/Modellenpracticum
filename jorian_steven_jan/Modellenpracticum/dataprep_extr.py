...
def last_extremas(df, time_increment, lookback, column):
    ...
    for i in range(extremas_ind[lookback + 1], len(df.index)):
        last_ind = extremas_ind[int(counter)]
        if i <= last_ind:
            array += [extremas[counter - lookback: counter]]   
        if (i > last_ind and counter < len(extremas_ind) - 1):
            counter += 1
            array += [extremas[counter - lookback: counter]]
    return array
             
print(last_extremas(df, 1.0, 2, 'z_velocity'))       
