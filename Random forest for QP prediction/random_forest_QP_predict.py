...
import features
from pca_lagged_variables import create_pca_lagged_features
...
def model_train(X, y, id=1, new = False):
    pickle_model_path = f'pickle_saves/modellen/model{str(id)}_RF.pkl'
    try:
        # Try to load the data if it's already saved
        if new:
            raise FileNotFoundError
...
# THIS PROGRAM NEEDS TO BE RUN FROM FOLDER Random forest for QP prediction
