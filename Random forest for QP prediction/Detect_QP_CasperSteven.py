...
def heave_speed(sw,su,h,y,r,p,pos,N, dt = None, new=False, name="heave_speed"):
    ...
    try:
        ...
        Heave_Speed = load_processed_data(f"pickle_saves/vectors/heave_speed_{name}.pkl")
        ...
        save_processed_data(np.array(Heave_Speed), f"pickle_saves/vectors/heave_speed_{name}.pkl")
...
def heli_incl(heave, sway, surge, yaw, roll, pitch, time, pos_helideck, new = False, name="heli_incl"):	
    try:
        if new:
            raise FileNotFoundError
        angle = load_processed_data(f"pickle_saves/vectors/heli_incl_{name}.pkl")
        ...
        save_processed_data(np.array(angle), f"pickle_saves/vectors/heli_incl_{name}.pkl")
...
def mark_QP(df, name="QP", new = False):
    try:
        ...
        QP = load_processed_data(f"pickle_saves/vectors/{name}.pkl")
        ...
        # print(df.head())
        # print(QPstart)
        save_processed_data(QP, f"pickle_saves/vectors/{name}.pkl")
