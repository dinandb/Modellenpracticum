...
error_new<- function(stps=1, t_stp=1, false_pos_weight=1.8, false_neg_weight=0.2, bar=0.05)
{
  predicted_probs_QPs <- logreg_predict_eval(stps, t_stp)
  predicted_probs = predicted_probs_QPs$probs
  QP_eval = predicted_probs_QPs$QPs
  logLoss(QP_eval, predicted_probs)
  return (logLoss(QP_eval, predicted_probs))
}
