...
def findMaxVals(heaves):
    # Find local maxima and minima
    peaks = signal.find_peaks(heaves)[0]
    troughs = signal.find_peaks(-heaves)[0]  # Negate the signal

    # Combine peaks and troughs into a single array
    combined_indices = np.concatenate((peaks, troughs))
    combined_values = np.concatenate((heaves[peaks], heaves[troughs]))

    # Sort the combined array by indices
...
def train_qp_predictor(X, y):
    ...
    y_pred = y_pred[:, 1] > 0.5  # Threshold at 0.8
