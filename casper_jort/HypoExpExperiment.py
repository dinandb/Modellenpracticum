...
#Hypoexpential experiment:
b = min(len(QPduration),len(QPbetweentimes))
QPcycle = np.add(QPduration[0:b],QPbetweentimes[0:b])

plt.hist(QPcycle,density=True,bins=30,label='Histogram')
x = np.arange(0,np.max(QPcycle),0.05)
l = 1/np.mean(QPduration_n)
m = 1/np.mean(QPbetweentimes)
plt.plot(x,((m*l)/(m-l)*(np.exp(-l*(x-30))-np.exp(-m*(x-30)))),label='Hypexp pdf')
plt.ylim(bottom=0)
plt.title('Empirical and theoretical distribution of X+Y+30')
plt.xlabel('time (s)')
plt.ylabel('density')
plt.legend()
